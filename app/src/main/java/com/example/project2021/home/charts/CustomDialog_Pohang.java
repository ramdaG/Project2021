...
public class CustomDialog_Pohang extends Dialog {
        ...
        btnSave.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (testTxt.getBytes().length <= 0) {
                    ...
                    mFirestore.collection("comments_Pohang").add(testMap).addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
                        @Override
                    ...
                    switch (radioGroup.getCheckedRadioButtonId()) {
                        case R.id.rb_Coat:
                            ...
                            myRef.child("Charts").child("Pohang").child(mCoat).child(uid).setValue("");
                            break;
                        case R.id.rb_Long:
                            System.out.println("b");
                            myRef.child("Charts").child("Pohang").child(mLong).child(uid).setValue("");
                            break;
                        case R.id.rb_Short:
                            System.out.println("c");
                            myRef.child("Charts").child("Pohang").child(mShort).child(uid).setValue("");
                            break;
                    }
    ...
    public CustomDialog_Pohang(Context mContext) {
        super(mContext);
